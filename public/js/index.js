...
function fetchChangelog(version) {
    if (localStorage.lrrVersion !== version) {
        ...
        fetch("https://api.github.com/repos/uparrows/LANraragi_cn/releases/latest", { method: "GET" })
            ...
            .catch((error) => { showErrorToast("获取新版本的变更日志时出错", error); failureCallback(error); });
    }
}
...
function handleContextMenu(option, id) {
    ...
    case "edit":
        ...
        break;
    case "delete":
        if (confirm("你确定删除该档案吗?")) deleteArchive(id);
        break;
    case "read":
...
function loadTagSuggestions() {
    ...
    genericAPICall("/api/database/stats?minweight=2", "GET", null, "无法加载标签建议",
        (data) => {
