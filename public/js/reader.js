...
Reader.toggleHelp = function () {
    ...
    $.toast({
        ...
        <div class="navigation-help-toast">
            ...
            <br>要返回存档索引，请触摸向下的箭头或使用Backspace.
        </div>
        `,
        hideAfter: false,
        position: "top-left",
...
Reader.goToPage = function (page, fromHistory = false) {
    ...
    if (Reader.trackProgressLocally) {
        ...
        genericAPICall(`api/archives/${Reader.id}/progress/${Reader.currentPage + 1}`, "PUT", null, "更新阅读进度时出错!", null);
    }
...
Reader.regenerateThumbnail = function () {
    ...
    if (!window.confirm("您确定要重新生成此存档的缩略图吗?")) {
        return;
    }
